...
export default function MyCarousel({ inside }) {
    return (
        <div className='relative'>
            <BackgroundImage />
            <div className="container w-full h-full px-4 pt-12 pb-20 mx-auto md:pb-36">
                <div className='mb-8 font-serif text-3xl font-semibold md:text-5xl'>{!inside ? <>Rooms & <span className="italic">Amenities</span></> : "Other Rooms at Greenway Stays"}</div>
                        ...
                        {rooms.map((item, index) => (
                            <CarouselItem className={cn('basis-[80%] sm:basis-1/2 md:basis-1/3 selection:bg-transparent', index == 0 ? 'pl-4' : 'pl-2 md:pl-8')} key={index}>
                                <div className='flex flex-col items-center relative w-auto h-[34rem] sm:h-[32rem] rounded-lg justify-end overflow-hidden group duration-500'>
                                    <h3 className='z-20 px-6 pb-4 font-serif text-xl font-light tracking-tighter text-white md:pb-6 md:text-3xl'>{item.category}</h3>
                                    <div className='z-20 flex items-center justify-center w-full gap-8 pb-6 font-sans text-xs tracking-tight text-white md:text-base font-extralight md:pb-8'>
                                        <Link href={`/rooms/${item.slug}`} className='px-4 py-2 duration-500 border rounded-full cursor-pointer hover:backdrop-blur-sm hover:bg-white/20 hover:border-white/20'>Learn more</Link>
                                        <div className='px-4 py-2 duration-500 border rounded-full cursor-pointer hover:backdrop-blur-sm hover:bg-white/20 hover:border-white/20'><ShoppingBag strokeWidth={1} size={20} /></div>
                                    </div>
